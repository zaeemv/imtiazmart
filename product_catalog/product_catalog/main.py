...
async def process_user_message(user_message: str, session: Session = Depends(get_session), client: OpenAI = Depends(get_client)):
    try:
        ...
        assistant = client.beta.assistants.create(
            instructions="You are a doctor who can use knowledge base to answer questions about the dental clinic and can also manage appointments for your patients. You can add or remove appointments from the database. Also, you need to reply in a single sentence.",
            model="gpt-4o-mini",
            tools=tools
        )
        ...
        while True:
            ...
            if runStatus.status == "requires_action":
                ...
                for message in messages.data:
                    ...
                    return(f"{role_label}: {message_content}\n")
                break 
